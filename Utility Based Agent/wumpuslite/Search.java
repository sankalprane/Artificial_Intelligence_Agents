    ...
    private List<State> generatePossibleStates(State currState, int x, int y) {
        ...
        if (currState.state[x][y] == 0) {
            for (int i = 0; i < possibleEnv.length; i++) {
                State newState = new State(currState.state, currState.direction, x, y);
    ...
    private void dfs(State currState, int depth, int utility, List<Integer> listOfMoves) {
        ...
        for (int i = 0; i < moves.length; i++) {
            ...
            if (moves[i] == Action.GO_FORWARD) {
                if (checkForwardMove(currState)) {
                    ...
                    for (int j = 0; j < listOfStates.size(); j++) {
                        dfs(listOfStates.get(j), depth + 1, listOfStates.get(j).getUtility(new_x, new_y), listOfMoves);
                    }
            ...
            if (moves[i] == Action.TURN_LEFT) {
                ...
                dfs(new State(currState.state, updateDirectionOnLeftMove(currState.direction), currState.x, currState.y), depth + 1, currState.getUtility(currState.x, currState.y), listOfMoves);
                listOfMoves.remove(listOfMoves.size() - 1);
            }
            if (moves[i] == Action.TURN_RIGHT) {
                listOfMoves.add(Action.TURN_RIGHT);
                dfs(new State(currState.state, updateDirectionOnRightMove(currState.direction), currState.x, currState.y), depth + 1, currState.getUtility(currState.x, currState.y), listOfMoves);
                listOfMoves.remove(listOfMoves.size() - 1);
            }
        }
    }
